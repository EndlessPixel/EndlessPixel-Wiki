# API开放文档 <Badge type="warning" text="beta" />
::: tip
...
::: tip
此接口返回结果与api.mcsrvstat.us相同，请按照api.mcsrvstat.us的文档解析返回。
:::
...
```json
{
  ...
  "teapot_mood": "silly",
  // 随机假错误信息
  "fake_error": "TeapotError: Out of tea (April Fools' Fake Error)",
  // 标记为愚人节接口
  "april_fools": true,
  // 当前时间
  "time": "2026-04-09T12:00:00.000Z",
  // 随机茶壶要求
  "teapot_demand": "Star 1 repo",
  // 随机茶壶建议
  "teapot_advice": "Always keep a teapot nearby when coding 🫖",
  // 假API状态
  "fake_api_status": "Running on teapot OS v4.18 ☕️",
  // 假客服信息
  "fake_support": "Teapot Tech Support: Call ☎️ 1-800-TEAPOT...",
  // 随机整蛊表情组合
  "prank_emojis": "🫖🎭🤪",
  // 整蛊广告：随机推荐7个仓库
  "ad": {
    "slogan": "Teapot's April Fools' Picks 🫖",
    "picks": [
      "Chat-Box [April Fools: Talks way too much 🗣️]: https://github.com/EndlessPixel/Chat-Box"
    ],
    "prank_disclaimer": "Disclaimer: 愚人节整蛊，不负责任 🎉",
    "teapot_rating": "4/5 cups of tea ☕️",
    "april_fools_challenge": "Star 3 repos before the teapot gets angry ⏰",
    "fake_downloads": "1234 (fake) downloads 📥"
  },
  // 假请求ID
  "fake_request_id": "TEAPOT-123456-APRILFOOLS",
  // 假加载耗时
  "fake_load_time": "1.23s (fake) ⏱️"
}
```

### 1. GET 请求
返回**基础整蛊信息** + **随机推荐7个GitHub仓库**
无额外字段，简洁版整蛊。

### 2. POST 请求
在GET基础上，**额外增加POST专属整蛊字段**：
```json
{
  // ... 通用字段同上 ...
  "post_prank": "Oops! This teapot drank your POST body 🫖",
  "fake_fix": "To fix: Pour tea on your server 😉",
  "fake_post_processing": "POST body converted to tea leaves 🍵",
  "fake_data_saved": "Data saved to teapot storage (50KB fake) 💾",
  "fake_response_delay": "5ms (we were making tea) ⏳"
}
```
...
```json
{
  ...
  "group_id": "870594910",
  // 群名称
  "group_name": "无尽像素 - EndlessPixel | 1.21.11原版纯净生存 | 冲刺！MC 26.1.1",
  // 群头像URL（标准尺寸100x100）
